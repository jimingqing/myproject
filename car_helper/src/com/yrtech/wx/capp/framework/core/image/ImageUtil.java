package com.yrtech.wx.capp.framework.core.image;
...
public class ImageUtil {  
     ...
     */  
    public static void resize(String srcImgPath, String outImgPath,  
            int maxLength, float per, boolean bb) {  
        // 得到图片  
        BufferedImage src = InputImage(srcImgPath);  
        ...
        BufferedImage newImg = new BufferedImage(maxLength, maxLength,  
                BufferedImage.TYPE_INT_RGB);  
        /*if (bb) {   
        ...
        newImg.getGraphics().fillRect(0, 0, maxLength, maxLength);
        newImg.getGraphics().drawImage(  
                tempImg.getScaledInstance(new_w, new_h, Image.SCALE_SMOOTH), 0, (maxLength-new_h)/2, null); 
        // 调用方法输出图片文件  
        outImage(outImgPath, newImg, per);  
    }  
     ...
     * @param height 高度, width 宽度
     * @param bb 比例不对时是否需要补白
*/
    public static void resize(String srcImgPath, String destImgPath, int width, boolean bb) {
        try {
        	System.out.println("srcImgPath: "+srcImgPath);
        	System.out.println("destImgPath: "+destImgPath);
            double ratio = 0; //缩放比例    
            BufferedImage bi = InputImage(srcImgPath);  
            ...
            Image itemp = bi.getScaledInstance(new_w, new_h, BufferedImage.SCALE_SMOOTH);   
            ...
            if (bb) {   
                ...
                if (width == itemp.getWidth(null))   
                    g.drawImage(itemp, 0, (width - itemp.getHeight(null)) / 2, itemp.getWidth(null), itemp.getHeight(null), Color.white, null);   
                else  
                    g.drawImage(itemp, (width - itemp.getWidth(null)) / 2, 0, itemp.getWidth(null), itemp.getHeight(null), Color.white, null);   
                g.dispose();   
                itemp = image;   
            ...
            File dfile = new File(destImgPath);
            if (!dfile.getParentFile().exists()) {
            	dfile.getParentFile().mkdirs();
			}
            // 调用方法输出图片文件  
            outImage(destImgPath, image, 1f); 
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void main(String args[]) throws Exception {  
        String srcImg = "F:/img/IMG_1446.JPG";  
        String tarDir = "F:/img/newImg/";  
        URL url = ImageUtil.class.getResource("IMG_1446.JPG");
...
//        resize(srcImg, tarDir + "car_1_maxLength_4-170x170.jpg", 350, 350);  
//        resize(srcImg, tarDir + "car_1_maxLength_11-350px.jpg", 400, 1F);  
        resize(srcImg, tarDir + "car_1_maxLength_12-350px.jpg", 800, true); 
