...
<script type="text/javascript">
	...
	function qryAuth(level, parentId){
		var param = {};
	...
	function actSubmit(){
		...
		if(confirm("确认修改权限！")){
			...
			doAjaxRequest("${ctx}/console/au_editAuthInfo.action", param, succCallBack, errCallBack);
		}
	}
	...
	function initAuthInfo(){
		...
		var succCallBack=function(data){
			if(data.retCode == "0000"){
				$("#authLevel").val(data.content.authLevel);
				if(data.content.parentId>-1){
					qryAuth(data.content.authLevel, data.content.parentId);
					$("#parent").show();
					$("#parentId").val(data.content.parentId);
				}
				$("#authDesc").val(data.content.authDesc);
				$("#porder").val(data.content.porder);
				$("#state").val(data.content.state);
				$("#isDefault").val(data.content.isDefault);
				$("#isToMer").val(data.content.isToMer);
				$("#isDisplay").val(data.content.isDisplay);
				$("#action").val(data.content.action);
			}else{
				alert("查询权限信息失败");
			}
		};
		...
		doAjaxRequest("${ctx}/console/au_qryAuthInfoDetail.action", param, succCallBack, errCallBack);
	}
