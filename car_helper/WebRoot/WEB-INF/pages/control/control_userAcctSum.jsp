  ...
  <table width="100%"  border="0" cellpadding="0" cellspacing="1" class="blue_kuang" style="margin-top:7px;">
								...
								<tr align="left" valign="bottom" class="black_text_k5">
									...
									<td width="35%" bgcolor="#FFFFFF" colspan="3">
										<input name="startDate" id="startDate" type="text" class="txt_input" size="10" value="<%=DateOper.getYesterday() %>" readonly="readonly" onClick="WdatePicker()">
										-
...
<!--查询区域外框开始结束-->
	...
	<div style="height:10px; width:auto;padding:10px;">
		<table id="header" width="80%" border=0 cellspacing="1" cellpadding="0" bgcolor="#C0C9E0" class="List">
			<thead>
			<tr>
				<td width="80" class="div">用户名</td>
				<td width="80" class="div">电话号码</td>
				<td width="150" class="div">交易笔数</td>
				<td width="150" class="div">账务日期</td>
		...
		var succCallBack=function(data){
			...
			if(data.retCode == "0000"){
				if(data.content.length > 0){
					...
					$.each(data.content, function(index, item){
						html += "<tr>";
						html += "<td align='center'>"+item.userName+"</td>";
						html += "<td align='center'>"+item.userCode+"</td>";
						html += "<td align='right'>"+item.transCnt+" 笔</td>";
						html += "<td align='right'>"+item.acctDate.replace("T"," ")+"</td>";
			...
			//http://localhost:8080/yrxsymp/console/as_qryUserAcctSumLog.action?merId=1
		doAjaxRequest("${ctx}/console/as_qryUserAcctSumLog.action", param, succCallBack);
	}
