...
**/
	...
	$(function(){
		...
		//$("#deptName").append("<option value='${userInfoVo.merId}'>${userInfoVo.merName}</option>");
		 // doAjaxRequest("${ctx}/console/mer_qryMerInfo.action", null, qryMerInfosuccCallBack);
		//alert('${userInfoVo.merProv}');
			...
			if(data.retCode == "0000"){
				...
				if(data.content.datalist.length > 0){
					...
					$.each(data.content.datalist, function(index, item){
						...
						if(item.userState == "A"){
							html += "<td align='left'><a href='${ctx}/admin/qryUserInfoDetailPage.action?userId="+item.id+"'>详情</a> | <a href='${ctx}/admin/editUserInfoPage.action?userId="+item.id+"'>修改</a> </td>";
